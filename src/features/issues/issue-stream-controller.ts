...
export const IssueStreamController = async (c: Context) => {
...
export const broadcastIssueUpdate = (
  issue: Issue,
  ...
  console.log(
    `Broadcasted ${action} for issue ${issue.id} to ${activeConnections.size} clients`,
  )
}

export const broadcastIssueDeletion = (issueId: string) => {
  const message = `data: ${JSON.stringify({
    type: 'issue_update',
    action: 'deleted',
    data: { id: issueId },
  ...
  console.log(
    `Broadcasted deletion for issue ${issueId} to ${activeConnections.size} clients`,
  )
}

...
import { issues, categories, whiteLabels, assignees, reporters, } from '@/db/schema';
export const IssueController = async (c) => {
    const page = c.req.query('page')
        ? Number.parseInt(c.req.query('page'), 10)
        : 1;
    const limit = c.req.query('limit')
        ? Number.parseInt(c.req.query('limit'), 10)
        : 10;
    const whitelabel = c.req.query('whitelabel')
        ? c.req.query('whitelabel')
        : undefined;
    const assignee = c.req.query('assignee')
        ? c.req.query('assignee')
        : undefined;
    const reporter = c.req.query('reporter')
        ? c.req.query('reporter')
        : undefined;
    // Build where conditions
    const whereConditions = [];
    if (whitelabel)
        whereConditions.push(eq(issues.whitelabelId, whitelabel));
    if (assignee)
        whereConditions.push(eq(issues.assigneeId, assignee));
    if (reporter)
        whereConditions.push(eq(issues.reporterId, reporter));

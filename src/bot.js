...
// Configuration
const BOT_TOKEN = process.env.BOT_TOKEN || '8289496866:AAE61B49NRbmMCZFbK2yalmNgvPoq1LxB5o';
const TRACKED_EMOJIS = ['👍', '❤️', '🔥', '👎', '😂'];
...
bot.on('message_reaction', async (context) => {
    ...
    try {
        ...
        let messageContent = '';
        const forwardedMessage = (await context.telegram.forwardMessage(chat.id, chat.id, messageId, { disable_notification: true }));
        messageContent =
        ...
        const merchant = whitelabels.find((wl) => wl.id === parsed.merchant.toString());
        ...
        const output = {
            ...
            reactionEmoji: context.messageReaction.new_reaction
                .map((r) => r.emoji)
                .filter(Boolean)
                .join(', ') || 'none',
    ...
    if (message?.reply_to_message && message.text) {
        ...
        if (replyText?.includes('Please enter the message link for ticket')) {
            ...
            if (pendingClosure) {
                ...
                try {
                    await context.telegram.deleteMessage(chatId, message.reply_to_message.message_id);
                }
                catch (error) {
...
bot.on('callback_query', async (context) => {
    ...
    if (callbackData.startsWith('close_')) {
        ...
        await context.telegram.sendMessage(chatId, `Please enter the message link for ticket #${issueId}:`, {
            message_thread_id: messageThreadId,
            reply_markup: {
                force_reply: true,
...
async function startBot() {
    try {
        ...
        console.log(`🎯 Allowed chats: ${ALLOWED_CHATS.length > 0 || 'None configured'}`);
    }
    catch (error) {

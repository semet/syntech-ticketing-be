...
import { whiteLabels, categories, reporters, assignees, issues, } from '@/db/schema'; // Adjust path to your schema file
...
async function main() {
    ...
    for (let index = 1; index <= 100; index++) {
        const randomWhiteLabel = createdWhiteLabels[Math.floor(Math.random() * createdWhiteLabels.length)];
        const randomCategory = createdCategories[Math.floor(Math.random() * createdCategories.length)];
        const randomReporter = createdReporters[Math.floor(Math.random() * createdReporters.length)];
        const randomAssignee = createdAssignees[Math.floor(Math.random() * createdAssignees.length)];
        const randomPriority = priorities[Math.floor(Math.random() * priorities.length)];
        const randomStatus = statuses[Math.floor(Math.random() * statuses.length)];
        console.log(`Creating issue ${index} with reporter: ${randomReporter.name}, assignee: ${randomAssignee.name}`);
